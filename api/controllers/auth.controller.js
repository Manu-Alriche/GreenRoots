  ...
  try {
    ...
    if (existingUser) {
      if (existingUser.email_verified) {
        ...
        return res.status(StatusCodes.CONFLICT).json({
          message:
            "Un compte avec cet email existe déjà mais n'est pas vérifié. Vérifiez votre email ou demandez un nouveau lien.",
  ...
  try {
    ...
    if (!user.email_verified) {
      return res.status(StatusCodes.FORBIDDEN).json({
        message:
          "Votre email n'est pas encore vérifié. Vérifiez votre boîte email ou demandez un nouveau lien.",
        needsVerification: true,
        email: user.email,
      });
    }
